...
AS $$
  SELECT jsonb_build_object(
    ...
    'edges', jsonb_build_array(
      ...
      jsonb_build_object('id', 'edge_menu_primary', 'source', 'menu', 'target', 'primary', 'condition', jsonb_build_object('type', 'contains', 'value', lower(p_primary_label), 'variable', null, 'label', p_primary_label, 'isFallback', false, 'priority', 0)),
      jsonb_build_object('id', 'edge_menu_secondary', 'source', 'menu', 'target', 'secondary', 'condition', jsonb_build_object('type', 'contains', 'value', lower(p_secondary_label), 'variable', null, 'label', p_secondary_label, 'isFallback', false, 'priority', 1)),
      jsonb_build_object('id', 'edge_menu_handoff', 'source', 'menu', 'target', 'handoff', 'condition', jsonb_build_object('type', 'contains', 'value', 'talk to team', 'variable', null, 'label', 'Talk to Team', 'isFallback', false, 'priority', 2)),
...
WITH seeds AS (
  SELECT * FROM (VALUES
    ...
    ('ecommerce_store','Ecommerce Store','🛍️',ARRAY['Retail','Ecommerce'],ARRAY['orders','returns','support'],'Browse products, track orders, and route returns.','shop','Welcome to our store. Choose an option below.','Browse Products','Add your catalog link here. Promotional outbound copy may require WhatsApp approval.','Track or Return','Share order ID and registered phone/email. Support will check status or return eligibility.','Connecting you to customer support.','Thanks for shopping with us.',true,false,true),
    ...
    ('education_coaching','Education / Coaching','🎓',ARRAY['Education'],ARRAY['courses','demo class','counsellor'],'Capture course enquiries and demo class requests.','course','Welcome. Choose an option below.','Courses and Fees','Add course list, duration, fees, and batch options. Availability is confirmed by admissions.','Demo Class','Share student name, course interest, preferred date, and phone number.','Connecting you to an admissions counsellor.','Thank you. Our counsellor will contact you soon.',false,false,false),
    ('gym_fitness_studio','Gym / Fitness Studio','🏋️',ARRAY['Fitness'],ARRAY['trial','membership','trainer'],'Promote trial sessions and membership plans.','fitness','Welcome. Choose an option below.','Membership Plans','Add plan details here. Promotional outbound messages may require WhatsApp approval.','Trial Session','Share your name, goal, and preferred time. Our team will confirm the slot.','Connecting you to the fitness desk.','Thanks. See you soon.',false,false,true),
    ('hotel_homestay','Hotel / Homestay','🏨',ARRAY['Hospitality'],ARRAY['rooms','booking','amenities'],'Capture room enquiries without implying confirmed booking.','room booking','Welcome. Choose an option below.','Rooms and Amenities','Add room types, amenities, check-in/out times, and policies. Rates and availability must be confirmed.','Check Availability','Share check-in date, check-out date, guest count, and room preference. Reservations will confirm.','Connecting you to reservations.','Thank you. Reservations will respond shortly.',false,false,false),
    ('travel_agency','Travel Agency','✈️',ARRAY['Travel'],ARRAY['packages','budget','callback'],'Collect destination, dates, and budget before advisor callback.','travel package','Welcome. Choose an option below.','Package Enquiry','Share destination, dates, travelers, and budget. Package prices and availability are confirmed by an advisor.','Visa or Flights','Share destination country, travel date, and passenger count. An advisor will guide you.','Connecting you to a travel advisor.','Thank you. We will get back with suitable options.',false,false,false),
    ...
    ('general_business','General Business','🏢',ARRAY['General'],ARRAY['services','pricing','support'],'Flexible starter flow for service businesses.','hi','Welcome. Choose an option below.','Services','Add your core services here and update this copy for your business.','Pricing or Enquiry','Share what you need, timeline, and contact details. Our team will respond with next steps.','Connecting you to our team.','Thank you for contacting us.',false,false,false)
