...
import { MessageSquare, HelpCircle, Image as ImageIcon, Film, FileText, Youtube, Zap, Trash2 } from 'lucide-react'
...
export default function SimpleNode({ data, selected }: NodeProps) {
  ...
  const isButtons = step.mode === 'button_choices'
  const hasTrigger = triggerKeywords.length > 0

  const preview = step.text.trim().slice(0, 80) || (isQuestion ? 'Untitled question' : 'Untitled message')
  const Icon = isQuestion ? HelpCircle : MessageSquare
      ...
      <div className="px-3 py-2.5">
        <div className="flex items-center justify-between gap-1.5 mb-1.5">
          <div className="flex items-center gap-1.5 min-w-0">
            <Icon className={`h-3.5 w-3.5 shrink-0 ${isQuestion ? 'text-primary' : 'text-sky-400'}`} />
            <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider truncate">
              {isQuestion ? (isButtons ? 'Question · buttons' : 'Question · open text') : 'Message'}
            </span>
          </div>
          <button
        ...
        {isButtons && step.buttons && step.buttons.length > 0 && (
